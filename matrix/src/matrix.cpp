...
void Matrix::resize(size_t new_rows, size_t new_cols) {
    ...
    for (size_t k = 0; k < new_cols * new_rows; ++k) {
        if (k < this->rows * this->rows) {
            new_matr[k / new_cols][k % new_cols] = this->matr[k / this->cols][k % this->cols];
        } else {
...
double Matrix::det() const {
    ...
    if (this->rows == 2) {
        return this->matr[0][0] * this->matr[1][1] - this->matr[0][1] * this->matr[1][0];
    }
    double d = 0.;
    int f = 1;
...
size_t task::Matrix::ncols() const {
    return this->cols;
};

size_t task::Matrix::nrows() const {
    return this->rows;
};
